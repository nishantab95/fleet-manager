...
export function DriverQaWorkspace() {
  ...
  const openKmReading = () => {
    if (dutyStatus !== "NONE" && dutyStatus !== "ACTIVE" && dutyStatus !== "CLOSED") return;
    setError("");
    setReadingType(dutyStatus === "ACTIVE" ? "END_READING" : "START_READING");
    setActiveAction("KM_READING");
  };
  const canStartOrEndDuty = Boolean(assignment) && !busy && (dutyStatus === "NONE" || dutyStatus === "ACTIVE" || dutyStatus === "CLOSED");
  const canOperate = Boolean(assignment) && !busy && dutyStatus === "ACTIVE";
  const disabledActionHint = dutyStatus === "CLOSED" ? "Previous duty completed. Record START KM to begin the next duty session." : dutyStatus === "NONE" ? "Record START KM first." : "Loading duty state…";
