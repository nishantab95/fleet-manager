...
export function DriverQaWorkspace() {
  ...
  const startDuty = () => { setError(""); setReadingType("START_READING"); setActiveAction("KM_READING"); };
  const endDuty = () => { setError(""); setReadingType("END_READING"); setActiveAction("KM_READING"); };
  return <main className="admin-shell">
    ...
    <div className="admin-layout single-column"><section className="content">
      ...
      <section className="stack"><h2>Driver operational actions</h2><div className="metric-grid qa-actions">{dutyStatus === "NONE" && <button disabled={!assignment || busy} onClick={startDuty} type="button">START DUTY</button>}{dutyStatus === "ACTIVE" && <><button disabled={!assignment || busy} onClick={() => void submitEvent("TRIP_COMPLETE")} type="button">TRIP COMPLETE</button><button disabled={!assignment || busy} onClick={() => { setError(""); setActiveAction("DIESEL"); }} type="button">DIESEL ISSUED / RECORDED</button><button disabled={!assignment || busy} onClick={endDuty} type="button">END DUTY</button></>}{dutyStatus === "CLOSED" && <span className="notice">Duty is closed for this session. Start a new session only after the next assignment/operational day.</span>}<button className="danger-action" disabled={!assignment || busy} onClick={() => void submitEvent("EMERGENCY")} type="button">EMERGENCY</button></div><p className="muted">Emergency sends one alert using the current driver, tipper, site, supervisor, company, and timestamp context. No category, description, or evidence is required.</p></section>
