...
function AssignmentsPanel({ assignments, drivers, supervisors, tippers, sites, run, request }: PanelProps & { assignments: Assignment[]; drivers: Person[]; supervisors: Person[]; tippers: Tipper[]; sites: Site[] }) {
  const [driverId, setDriverId] = useState(""); const [supervisorId, setSupervisorId] = useState(""); const [tipperId, setTipperId] = useState(""); const [siteId, setSiteId] = useState(""); const [startsAt, setStartsAt] = useState(() => new Date().toISOString().slice(0, 16)); const [regularDutyHours, setRegularDutyHours] = useState("10");
  return <><h2>Assignments</h2><p className="muted">Regular duty is snapshotted when the driver starts duty; changing a future assignment does not alter a closed session.</p><form className="form-grid" onSubmit={(event) => { event.preventDefault(); void run(async () => { await request("/api/v1/admin/assignments", { method: "POST", body: JSON.stringify({ driver_membership_id: driverId, supervisor_membership_id: supervisorId, tipper_id: tipperId, site_id: siteId, starts_at: new Date(startsAt).toISOString(), regular_duty_minutes: Number(regularDutyHours) * 60 }) }); }); }}><Select label="Driver" value={driverId} onChange={setDriverId} options={drivers.map((person) => [person.membership_id, person.display_name])} /><Select label="Supervisor" value={supervisorId} onChange={setSupervisorId} options={supervisors.map((person) => [person.membership_id, person.display_name])} /><Select label="Tipper" value={tipperId} onChange={setTipperId} options={tippers.map((tipper) => [tipper.id, tipper.registration_number])} /><Select label="Site" value={siteId} onChange={setSiteId} options={sites.map((site) => [site.id, site.name])} /><label>Starts at<input type="datetime-local" value={startsAt} onChange={(event) => setStartsAt(event.target.value)} required /></label><label>Regular duty hours<select value={regularDutyHours} onChange={(event) => setRegularDutyHours(event.target.value)}><option value="8">8 hours</option><option value="10">10 hours</option><option value="12">12 hours</option></select></label><button type="submit">Create assignment</button></form><div className="table-card">{assignments.map((assignment) => <div className="table-row" key={assignment.id}><strong>{assignment.driver_name} · {assignment.registration_number}</strong><span>{assignment.site_name}</span><span>{assignment.supervisor_name}</span><span>{assignment.regular_duty_minutes / 60}h regular duty</span><span>{assignment.ends_at ? "Closed" : "Active"}</span>{!assignment.ends_at && <button className="secondary" type="button" onClick={() => void run(async () => { await request(`/api/v1/admin/assignments/${assignment.id}/close`, { method: "POST", body: JSON.stringify({ ends_at: new Date().toISOString() }) }); })}>Close</button>}</div>)}</div></>;
}
