...
| Python | Official Python Launcher-backed CPython 3.12 |
...
Use the already-installed official Python 3.12 launcher for this project:
...
The bootstrap script verifies prerequisites, creates `.env` if absent, and creates or verifies `services/api/.venv` using the trusted Python 3.12 interpreter. uv is retained only for locked dependency synchronization and is explicitly prevented from downloading another Python. Dependencies are pinned in `services/api/pyproject.toml` and `services/api/uv.lock`.
